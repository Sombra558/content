"""Rebuild ES Unit 6 slides to match EN Unit 6 order (EN = source of truth)."""
...
MARKERS: list[tuple[int, str, str]] = [
    ...
    (30, "Know when to escalate a situation", "cuándo escalar una situación"),
    ...
    (35, "Responsible promotions focus on", "promociones responsables se centran"),
    ...
    (38, "Watch for risks during promotions", "riesgos durante las promociones"),
    ...
    (62, "Support ethical service by", "Apoya el servicio ético"),
    ...
    (66, "Maintain professionalism by", "Mantén el profesionalismo"),
...
def main():
    ...
    es_sec = unit_section(es, 6)
    en_sec = unit_section(en, 6)
    en_slides = en_sec["slides"]
    es_slides = es_sec["slides"]
    assert len(en_slides) == len(es_slides) == 74

    by_i = {i: (i, a, b) for i, a, b in MARKERS}
    markers = [by_i[i] for i in range(74)]
    ...
    print(f"\nFinal mapped {len(mapping)}/74 orphans={orphans} unmapped={unmapped}")
    print("\n=== MOVES ===")
    for i in range(74):
        if i not in mapping:
            continue
    ...
    if len(mapping) != 74 or orphans or unmapped:
    ...
    new_slides = [copy.deepcopy(es_slides[mapping[i]]) for i in range(74)]
    ...
    slides = unit_section(es2, 6)["slides"]
    assert len(slides) == 74
    assert all(s["order_index"] == i for i, s in enumerate(slides))
    assert all(slides[i]["content_type"] == en_slides[i]["content_type"] for i in range(74))
    print("\nWrote ES Unit 6 remapped to EN order.")
    for i in [1, 2, 12, 22, 23, 24, 30]:
